...
**[NEEDS: Figure 4 — RGB and semantic renders vs. ground truth on held-out views.]** A grid of
...
by predicted class. This is secondary to Figures 4 and 5 — it illustrates the learned 3D structure
...
- [ ] Produce the remaining qualitative figures listed in 5.3 (Figures 4-6;
  `src/gaussian_splatting/render.py` + `render_metrics.py`'s per-view outputs already provide the
  raw material), and Figures 1-2 in Section 3 (pipeline diagram, cable-voting illustration).
