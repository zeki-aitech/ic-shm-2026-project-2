"""
pycolmap-based reconstruction (uses the CUDA-enabled pycolmap-cuda12 build).

Provides a drop-in alternative to the hand-written ColmapParser:

1. ``PycolmapReconstructor`` loads the contest COLMAP text model (which ships
   without ``points3D.txt``), rebuilds feature tracks from the 2D observations
   in ``images.txt`` and triangulates every track robustly with LO-RANSAC
   (``pycolmap.estimate_triangulation``, includes cheirality and minimum
   triangulation-angle checks).
2. ``reconstruction_to_parser_data`` converts any ``pycolmap.Reconstruction``
   into the ColmapParser-compatible dataclasses, so ``SemanticProjector`` and
   the visualizer can consume pycolmap results unchanged.
"""
import os
import shutil
import tempfile
...
def reconstruction_to_parser_data(
    ...
    """Convert a pycolmap.Reconstruction into ColmapParser-compatible structures."""
...
def filter_outliers_iqr(points3d: Dict[int, Point3D], iqr_multiplier: float = 3.0) -> int:
    """Same IQR distance filter as ColmapParser.filter_outliers. Returns removed count."""
    if not points3d:
        return 0
...
class PycolmapReconstructor:
    """
    Drop-in replacement for ColmapParser backed by pycolmap.

    Exposes the same load() -> (camera, images, points3d) contract, so it can be
    passed to SemanticProjector via its `parser` argument.
    """
...
class ReconstructionAdapter:
    """
    Wraps an existing pycolmap.Reconstruction (e.g. the output of the GPU
    re-triangulation pipeline) behind the same load() contract.
    """

    def __init__(self, rec: pycolmap.Reconstruction, iqr_multiplier: float = 3.0):
    ...
    def load(self) -> Tuple[CameraIntrinsics, Dict[int, ImagePose], Dict[int, Point3D]]:
        self.camera, self.images, self.points3d = reconstruction_to_parser_data(self.reconstruction)
        n_removed = filter_outliers_iqr(self.points3d, self.iqr_multiplier)
        print(f"[pycolmap] Adapter: {len(self.points3d)} points "
              f"({n_removed} IQR outliers removed) from {len(self.images)} images")
        return self.camera, self.images, self.points3d
